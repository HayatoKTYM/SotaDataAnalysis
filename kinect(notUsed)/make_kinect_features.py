...
class MakeKinectFeatures(object):
    ...
    def make_features(self, SWAY=False, FACE=False):
        ...
        if FACE:
            ...
            print faceA[0]
            print faceB[0]
        if not SWAY and FACE:
            kinect_info = []
            count = 0
            #for sA, sB, fA, fB in zip(swayA, swayB, faceA, faceB):
            #print(faceB)
            for fA, fB in zip(faceA, faceB):
    ...
    def calculate_face_info(self, face_list, person):
        ...
        t = self.frame_unit
        init_flag = True
        face_info_list = [["pitch_" + person,"delta_pitch_" + person, "yaw_" + person,"delta_yaw_" + person]]
        pitch = 0.0
        prepitch = 0.0
        yaw = 90.0
        preyaw = 0.0
            ...
            for v in face_list:
                ...
                if v[0] > t:
                    if init_flag:
                        ...
                        face_info_list.append([pitch, 0.0, yaw, 0.0])
                        init_flag = False
                        if not v[1] == "none":
                            prepitch = v[1]
                            preyaw = v[2]
                        ...
                        delta_yaw = abs(yaw - preyaw)
                        face_info_list.append([pitch,delta_pitch, yaw,delta_yaw])
                        prepitch = pitch
                        preyaw = yaw
                    is_updated = True
                    break
    ...
    def get_face_info(self):
        ...
        for t, f_id, p, y in zip(self.data["Time"], self.data[" faceindex"], self.data[" face_pitch"], self.data[" face_yaw"]):
            ...
            if self.start_time <= time and time <= self.end_time:
                ...
                #    yaw = abs(float(y) - 35.0)
                if not p == "none":
                    pitch = float(p)
                if f_id == self.Aid:
                    if not y == "none":  # yaw (The relative angle with Sota)
                        yaw = abs(float(y)) - 30.0
...
if __name__ == '__main__':
    ...
    for i in directory:
        ...
        for num in number:
            ...
            print MK.duration
            print "****************"
            MK.make_features(SWAY=False, FACE=True)
